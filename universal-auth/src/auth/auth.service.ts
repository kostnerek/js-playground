...
export class AuthService {
  ...
  async register(input: RegisterDto): Promise<User> {
    ...
    const { id, token } = await this.tokenService.generateVerifyAccountToken(
      user._id.toString(),
    );
    const verifyLink = `${this.serverConfig.getFrontendUrl()}/auth/verify-account?id=${id}&token=${token}`;
    this.eventEmitter.emitUserVerifyAccount({
  ...
  async resetPassword(email: string) {
    ...
    const { id, token } = await this.tokenService.generatePasswordResetToken(
      user._id.toString(),
    );
    const resetLink = `${this.serverConfig.getFrontendUrl()}/auth/reset-password?id=${id}&token=${token}`;
    this.eventEmitter.emitUserResetPassword({
      email: user.email,
      url: resetLink,
    });
  }
  ...
  async changePassword(body: ChangePasswordDto) {
    ...
    await this.userService.upsert({
      id: decodedToken.userId,
      hash: await bcryptHash(body.password, this.config.getSaltRounds()),
      authType: user.authType,
    });
    await this.tokenService.invalidateToken(body.id);
  }

  serializeExternalAuthData(
    userData: IGoogleUserData | IFacebookUserData,
  ): IExternalAuthUserData {
    return {
      email: 'email' in userData ? userData.email : '',
      firstName:
        'given_name' in userData
          ? userData.given_name
